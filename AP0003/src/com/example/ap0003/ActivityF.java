...
public class ActivityF extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Log.d(TAG, "ActivityF: onCreate(" + getTaskId() + ")");
	...
	protected void onStart() {
		super.onStart();
		Log.d(TAG, "ActivityF: onStart("+getTaskId()+")");
	}
	...
	protected void onResume() {
		...
		setTitle(getResources().getString(R.string.app_name) + " | "
				+ getLocalClassName() + " | TaskID: " + getTaskId());
		Log.d(TAG, "ActivityF: onResume("+getTaskId()+")");
		...
		for (RunningTaskInfo task : list) {
			if (task.id == getTaskId()) {
				...
				// Активностей
				tvTextLife = (TextView) findViewById(R.id.textActCountF);
	...
	protected void onPause() {
		...
		tvTextLife = (TextView) findViewById(R.id.textStateActF);
		// присваиваем значение атрибуту Text для выбранного TextView
		tvTextLife.setText("Этот экземпляр ActivityF уже был запущен!");
		Log.d(TAG, "ActivityF: onPause("+getTaskId()+")");
	}

	@Override
	protected void onStop() {
		super.onStop();
		Log.d(TAG, "ActivityF: onStop("+getTaskId()+")");
	}

	@Override
	protected void onRestart() {
		super.onRestart();
		Log.d(TAG, "ActivityF: onRestart("+getTaskId()+")");
	}

	@Override
	protected void onDestroy() {
		super.onDestroy();
		Log.d(TAG, "ActivityF: onDestroy("+getTaskId()+")");
	}
